...
(function() {
    ...
    const STORAGE_KEY_COMMENT_CARDS = 'moment_comment_cards_library';
    ...
    function initMomentsData() {
        ...
        try {
            ...
            commentCardsLibrary = cc ? JSON.parse(cc) : [];
        } catch(e) { commentCardsLibrary = []; }
        try {
    ...
    function saveMomentsData() {
        try {
            if (momentsData.length > MAX_MOMENTS) {
                momentsData = momentsData.slice(0, MAX_MOMENTS);
            }
            localStorage.setItem(STORAGE_KEY_MOMENTS, JSON.stringify(momentsData));
        } catch(e) {}
    }
    ...
    function saveCommentCardsLibrary() {
        try { localStorage.setItem(STORAGE_KEY_COMMENT_CARDS, JSON.stringify(commentCardsLibrary)); } catch(e) {}
    }
    ...
    function autoInteract() {
        ...
        if (Math.random() * 100 < autoSettings.commentChance) {
            ...
            if (commentCardsLibrary.length > 0) {
                comment = commentCardsLibrary[Math.floor(Math.random() * commentCardsLibrary.length)].text;
            }
            const partnerName = (typeof settings !== 'undefined' && settings.partnerName) ? settings.partnerName : '梦角';
    ...
    function addCommentCard(text) {
        if (!text.trim()) return;
        commentCardsLibrary.push({ id: 'cc_' + Date.now(), text: text.trim() });
        saveCommentCardsLibrary();
    }

    function removeCommentCard(cardId) {
    ...
    function renderMomentsList() {
        ...
        container.innerHTML = momentsData.map(moment => {
            ...
            const avatarUrl = getMomentAvatar(isMine ? 'me' : 'partner');
            ...
            return `<div class="moments-item">
                <div class="moments-item-header">
                    <div class="moments-avatar ${isMine ? 'moments-avatar-mine' : 'moments-avatar-partner'}" style="${avatarUrl ? `background-image:url(${avatarUrl});background-size:cover;` : ''}cursor:pointer;" onclick="event.stopPropagation();uploadMomentAvatar('${isMine ? 'me' : 'partner'}')">${!avatarUrl ? '<i class="fas fa-user"></i>' : ''}</div>
    ...
    function removePublishImage(index) {
        ...
        const preview = document.getElementById('moments-publish-images-preview');
        if (preview) {
            preview.innerHTML = overlay._publishImages.map((file, idx) => {
                const url = URL.createObjectURL(file);
                return `<div class="moments-publish-image-item"><img src="${url}"><button onclick="window.removePublishImage(${idx})">×</button></div>`;
            }).join('');
        }
    }
        ...
        fetch(img.dataUrl).then(res => res.blob()).then(blob => {
            ...
            if (overlayEl) {
                if ((overlayEl._publishImages || []).length >= 9) {
                    if (typeof showNotification === 'function') showNotification('最多9张', 'warning');
                    return;
                }
    ...
    function updateAutoSetting(key, value) {
    ...
    function openCommentCardsManager() {
        ...
        modal.innerHTML = `<div class="moments-card-picker" style="max-width:500px;"><div class="moments-card-picker-header"><span>💬 评论库</span><button onclick="document.getElementById('comment-cards-manager-overlay').remove()"><i class="fas fa-times"></i></button></div><div style="display:flex;gap:8px;padding:12px;"><input type="text" id="new-comment-card-input" placeholder="输入新评论内容..." style="flex:1;padding:10px;border:1px solid var(--border-color);border-radius:8px;font-size:13px;background:var(--primary-bg);color:var(--text-primary);outline:none;"><button onclick="window.addCommentCardAndRefresh()" style="padding:10px 16px;background:var(--accent-color);color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:600;">添加</button></div><div class="moments-card-picker-grid">${commentCardsLibrary.map(card => `<div class="moments-card-picker-item" style="display:flex;justify-content:space-between;align-items:center;"><span>${card.text}</span><button onclick="window.removeCommentCardAndRefresh('${card.id}')" style="background:none;border:none;color:#ff5050;cursor:pointer;">×</button></div>`).join('')}</div></div>`;
        modal.onclick = function(e) { if (e.target === modal) modal.remove(); };
        document.body.appendChild(modal);
    }
    ...
    function addCommentCardAndRefresh() {
        ...
        if (!input || !input.value.trim()) return;
        addCommentCard(input.value.trim());
    ...
    window.updateAutoSetting = updateAutoSetting;

    initMomentsData();
    console.log('📱 朋友圈模块已加载');
})();
